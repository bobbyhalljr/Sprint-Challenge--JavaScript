...
const universities = graduates.map((college) => {
...
const contactInfo = graduates.map((info) => {
  info = `${info.first_name} ${info.email}`;
  return info;
});
...
const uni = graduates.filter((u) => {
  if(u.valueOf('Uni')) {
    return console.log(u.university)}
});
...
const animalNames = zooAnimals.forEach((name) => {
  name = `${name.animal_name}, ${name.scientific_name}`;
...
const lowPop = zooAnimals.filter((lowPop) => { return lowPop.population < 5;
});
